...
function renderField(name, props) {
  var fieldConfig = (0,build_module/* select */.Lt)(store).getProductField(name);
...
function registration_registerProductField(fieldName, settings) {
  if ((0,build_module/* select */.Lt)(store).getProductField(fieldName)) {
    // eslint-disable-next-line no-console
    console.error('Product Field "' + fieldName + '" is already registered.');
    return;
  }
  (0,build_module/* dispatch */.JD)(store).registerProductField(registration_objectSpread({
    attributes: {}
  }, settings));
  return (0,build_module/* select */.Lt)(store).getProductField(fieldName);
}
